 ...
 * @param {string} jsPath
 */
export async function transformJs(jsPath, { bug, dryRun, root, title } = {}) {
  ...
  for (const props of properties) {
    if (props.ftl) {
      ...
      let pyPath = ''
      if (n > 0) {
        applyMigration(props)
        if (!dryRun) {
          await writeFile(fp, serializeFluent(props.ftl))

          if (props.ast.some((node) => node.type === 'PAIR')) {
          ...
          const ftlName = basename(props.ftlPath, '.ftl')
          ...
          await writeFile(pyPath, stringifyTransform(props, pyTitle))
          await execFile('python', ['-m', 'black', pyPath])
        }
