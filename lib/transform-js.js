...
export async function transformJs(jsPath, options = {}) {
  ...
  for (const { ftl, hasMigrateConfig, path } of properties) {
    ...
    } else console.warn(`Skipping ${relPath} (No FTL metadata)`)
  }
  if (migrationCount === 0) {
    console.error(`
...
`)
  ...
  for (const propData of properties) {
    if (propData.hasMigrateConfig) {
      ...
      await writeMigrationConfig(options.root, propData)
    }
  }
  if (migratedUris.length === 0) return
  ...
  for (const { path, bundleTags, src } of xhtml) {
    ...
    if (changedSrc !== src) {
      if (!options.dryRun) await writeFile(path, changedSrc)
  ...
  // Update JS file
  if (!options.dryRun) await writeFile(jsPath, print(ast).code)
