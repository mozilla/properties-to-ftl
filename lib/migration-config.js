...
export async function readMigrationConfig(root, propPath) {
  ...
  if (
    ...
    throw new Error(
      `Invalid meta value in ${relPath}, expected { bug: string, title: string }`
    )
  }
  if (
    !ftl ||
 ...
 */
export async function writeMigrationConfig(propData, { bug, root }) {
 ...
 It may be safely removed after the migration has been completed.`
  ...
  doc.get('migrate').items = msgKeys.map((propKey) => {
    ...
    if (migrate.hasOwnProperty(propKey)) {
      ...
      const { key, attr, varNames } = migrateMessage(propData, propKey, null)
      const comment = { key }
