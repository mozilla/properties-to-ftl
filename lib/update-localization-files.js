 ...
 *   - `.py` migration script
 *
 * @param {import('./external-refs').PropData} propData
 * @param {import('./transform-js').TransformOptions} options
 * @returns {boolean} If `true`, at least one string was migrated.
...
export async function updateLocalizationFiles(
  ...
  if (dryRun) {
    ...
    stringifyTransform(propData, 'TITLE')
  } else {
    ...
    await writeFile(pyPath, stringifyTransform(propData, pyTitle))
