...
export async function parseMessageFiles(path, options) {
  ...
  const msgKeys = ast
    ...
    .map((pair) => pair.key)
  const cfg = await readMigrationConfig(options.root, path)
  let ftlRoot, ftlPath, ftlPrefix, migrate
  if (cfg) {
    ...
    const meta = getFtlMetadata(path, ast, options)
    ftlRoot = meta.ftlRoot
    ftlPath = meta.ftlPath
    ftlPrefix = meta.ftlPrefix
    migrate = {}
  }
