...
export async function parseMessageFiles(path, options) {
  const ignore = options.ignore.some((rule) => path.endsWith(rule))
  const src = await readFile(path, 'utf8')
  const ast = parseLines(src, true)
  let msgKeys = ast
    .filter((node) => node.type === 'PAIR')
    .map((pair) => pair.key)
  const cfg = ignore ? null : await readMigrationConfig(options.root, path)
  ...
  const ftl = ignore ? null : await getFluentResource(ftlRoot, ftlPath)
  return {
